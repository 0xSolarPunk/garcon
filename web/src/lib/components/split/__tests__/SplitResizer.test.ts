...
function renderResizer(direction: SplitDirection, ratio = 0.5) {
	const onResizeStart = vi.fn();
	const onResize = vi.fn();
	const onReset = vi.fn();
	render(SplitResizer, { direction, ratio, onResizeStart, onResize, onReset });
...
describe('SplitResizer', () => {
	...
	it('exposes the current split ratio through ARIA values', () => {
		renderResizer('horizontal', 0.62);
